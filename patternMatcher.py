# O(n2 + m) time | O(n+m) space
def patterMatcher(pattern, string):
    ...
    if counts["y"] != 0:
        for lenOfX in range(1, len(string)):
            lenOfX = (len(string) - lenOfX * counts["x"]) / counts["y"]
            if lenOfY <= 0 or lenOfY % 1 != 0:
                continue
            lenOfY = int(lenOfY)
            yId = firstYPos * lenOfX
            x = string[:lenOfX]  # 'g"
            y = string[yId: yd + _ lenOfY]  # "gopowerranger"
